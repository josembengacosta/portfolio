...
async function networkFirstWithOffline(request) {
  try {
    ...
    return new Response(`
      ...
      <html><body>
        <h1>Offline</h1>
        <p>Conecte-se à internet para acessar ${new URL(request.url).pathname}</p>
      </body></html>
    `, {
      status: 503,
...
async function cacheSecondaryResources() {
  try {
    ...
    if (toCache.length > 0) {
      console.log(`📦 Cacheando ${toCache.length} recursos secundários em background...`);
        ...
        .then(keys => {
          event.source.postMessage({
            ...
            categories: {
              ...
              fonts: keys.filter(k => 
                k.url.match(/\.(woff|woff2|ttf|eot)$/i)
              ).length
            }
          });
        });
...
console.log(`Service Worker v${APP_VERSION} carregado para estrutura complexa!`);